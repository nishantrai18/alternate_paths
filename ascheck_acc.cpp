...
int main()
{
	...
	*/
	...
	char* files[]={"test1.gr","test2.gr","test3.gr","test4.gr","test5.gr"};

	int cot=4, tot_altp[110];
	pair < double, double > counts[110];
	...
	while(cot>=0)
	{
		...
		*/
		...
		while(j<distas.size())
		{
			...
			cout<<((i-1)*1000)<<"-"<<(i*1000)<<" :  "<<(tot/ct)<<"  "<<1-(fl/ct)<<endl;
	...
	for(i=1;i<100;i++)
	{
		cout<<((i-1)*1000)<<"-"<<(i*1000)<<" :  "<<counts[i].first<<"  "<<counts[i].second/5<<endl;
	}
	cout<<"\n";
...
int alt_path(int na, int nb, float share, float stretch, float localt)
{
	...
	*/	
	...
	altp_dist[cnt]+=1;
	//cout<<"THE TOTAL WASTED CANDIDATES WERE "<<kora<<"\n";
