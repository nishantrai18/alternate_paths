...
int main(int* argc, int** argv)
{
    ...
    FILE* f=fopen("bl_ny","r");
    FILE* fa=fopen("res_ny","r");
    FILE* fb=fopen("resas_ny","r");
    ...
    while(k>0)
    {
        ...
        for(i=0;i<a;i++)
        {
            fscanf(f,"%lf %lf",&bp[i].first,&bp[i].second);
            nrst.push_back(bp[i]);
        }
        for(i=0;i<n;i++)
        {
            fscanf(fa,"%lf %lf",&rp[i].first,&rp[i].second);
        }
        for(i=a-1;i>0;i--)
        {
            ponts tmp=nearest(bp[i],bp[i-1],rp,n,ids);
            nrst.push_back(tmp);
        }

        double mina=100000;
        mat[0][0]+=area(nrst);
        mat[1][0]+=dist_nearest(rp,bp,a,ids,mina);
        mat[2][0]+=mina;
