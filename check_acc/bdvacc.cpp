...
double altp=0,totp=0,giters=0,initp=0,gstretch=0,altp_dist[1100];
...
int main()
{
	...
	*/
	...
	double telavg=0,savg=0,apavg=0,inavg=0,acavg=0,itavg=0,stravg=0;

	int step_sz=500*(sz/3000)*(sz/3000)*(sz/3000);
	...
	cout<<"THE AVERAGE STRETCH IS : "<<stravg/5<<"\n";
	...
	*/
	...
	cout<<"THE ANALYSIS FOR THE T TEST IS : "<<endl;

	for(i=0;i<2;i++)
	{
		for(j=0;j<2;j++)
		{
			cout << ((ttest[i][j]*(1.0))/total_calls) <<" ";
		}
		cout<<endl;
	}
...
float bidijkstra( float limit, int na, int nb )
{
	...
	while ((!qa.empty())&&(!qb.empty())&&(ra+rb)<(ans*limit))
	{
		edge p = qa.top();
		int u = p.end_node;
		//cout<<p.end_node<<" : "<<p.weight<<" A\n";
				   ...
				   float share, float stretch, float localt)
{
	...
	int prop=0;
	//prop = proper_ttest( pathacb, index_nc, na, nc, nb, localt);
