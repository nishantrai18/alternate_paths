...
double compute_mid(double l, double r, double* shr, double* st, int na, int nb,map <double, ponts> &opti);
...
void plotty(int na, int nb)
{
	...
	while(!stk.empty())
	{
		...
		stk.pop();
		m=compute_mid(c.first,c.second,&shr,&st,na,nb,opti);
		if(m>(-1))
		{
...
double compute_mid(double l, double r, double* shr, double* st, int na, int nb, map <double, ponts> &opti)
{
	...
	ponts a,b;
	if(opti.find(l)==opti.end())
	{
		a=values(l,na,nb);
		opti.insert(make_pair(l,a));
		//cout<<"Saved, ";
	}
	else
	{
	...
	if(opti.find(r)==opti.end())
	{
		b=values(r,na,nb);
		opti.insert(make_pair(r,b));
		//cout<<"Saved, ";
	}
	else
	{
...
float biastar_forlength(float forcost[], float revcost[], float dista[], float distb[], int visa[], int visb[], int na, int nb, int* opv )
{
	...
	//cout<<ans+forcost[na]-forcost[nb]<<endl;
