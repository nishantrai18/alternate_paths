...
float biastar(float forcost[], float revcost[], int na, int nb );
int penalise(vector <int> path, int na, int nb);
...
int alt_path(int na, int nb)
{
	...
	opt=biastar(forcost, revcost, na, nb);
...
int penalise(vector <int> optpath, int na, int nb)
{
	int i,j,t,index_nc=0;
	
	for(i=0;i<optpath.size()-1;i++)
	{
		lengths[make_pair(optpath[i],optpath[i+1])]*=2;
	}
	return 0;
}
