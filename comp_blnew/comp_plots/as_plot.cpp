...
int main(int argc, char **argv)
{
	...
	FILE *wr=fopen(argv[1],"w");							//argument 1 is the file to be written into

	int S,T;

	S = atoi(argv[2]);										//argument 2 is the first node		
	T = atoi(argv[3]);										//argument 3 is the second node
...
void stack_dfs(int fg, int n, vector <int> optpath, vector <vector <edge> > arr, float share_amt[])
{
	...
	while(!stk.empty())
	{
		...
		if(binary_search(optpath.begin(), optpath.end(), n))
		{
			...
			share_amt[n]+=lengths[p];
			//cout<<n<<" FORA "<<fora[n]<<" PREV "<<prev[n]<<" WITH "<<lengths[p]<<",";
		}
		for(j=0;j<arr[n].size();j++)
		{
