...
int alt_path(int na, int nb, float share, float stretch, float localt)
{
	...
	for(int y=0;y<optpath.size();y++)
	{
		cout<<optpath[y]<<"|";
		if(y<optpath.size()-1)
			cout<<lengths[make_pair(optpath[y],optpath[y+1])]<<"  ";
	}	
	cout<<" WAS THE SHORTEST PATH\n";
	*/
	...
	/*for(int y=0;y<optpath.size();y++)
	{
		cout<<share_amt_fora[optpath[y]]<<"|";
	}	
	cout<<" THE FORA SHARE\n";
	for(int y=0;y<optpath.size();y++)
	{
		cout<<share_amt_prev[optpath[y]]<<"|";
	}	
	cout<<" THE PREV SHARE\n";	
	*/
			...
			for(int tr=0;(!t)&&(tr<partner.size());tr++)
			{
				...
				if(t==1)
				{
					...
					/*for(int y=0;y<pathacb.size();y++)
					{
						cout<<pathacb[y]<<" ";
						if(y<pathacb.size()-1)
							cout<<lengths[make_pair(pathacb[y],pathacb[y+1])]<<"  ";
					}	
					cout<<"\n";
					cout<<param_stretch<<" AND "<<param_share<<" AND "<<param_share*opt<<endl;
					*/
...
void stack_dfs(int fg, int n, vector <int> optpath, vector <vector <edge> > arr, float share_amt[])		//Earlier an error was present in the code
{
	int j,t=1;
	stack <int> stk;
	stk.push(n);
