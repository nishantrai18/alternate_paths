...
int main(int argc, char **argv)
{
	...
	FILE* f;
	//f=fopen(argv[1],"r");											//Read a file such as Romein
	FILE* fw = fopen(argv[1],"w");									//Output file
...
void plotty(int na, int nb)
{
	sz_pt=0;
	int i,j;
	double l=0,r=sqrt((double)INF2*10),m;
...
ponts values(double p, int na, int nb)
{
	...
	st=(st/origopt);				//st is modified in comp_share
	...
	cout<<"\n";
	cout<<st<<" AND "<<shr<<" AND "<<shr*origopt<<endl;
	*/
